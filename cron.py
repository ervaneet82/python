# Overrides directory.py default check to track more than 20 files - mgarcia 09/09/2017
...
class DirectoryCheck(AgentCheck):
	...
	Config options:
		...
		"filegauges" - boolean, when true stats will be an individual gauge per file (max. 50 files!) and not a histogram of the whole directory. default False
		...
		for root, dirs, files in walk(directory):
			for filename in files:
				...
				try:
					...
					if filegauges and directory_files <= 50:
						...
						self.gauge("system.disk.directory.file.created_sec_ago", time.time() - file_stat.st_ctime,
						           tags=filetags)
						self.gauge("system.disk.directory.file.locked_sec_ago", self._lock_active_time(filename),
						           tags=filetags)
					elif not filegauges:
	...
	def _lock_active_time(self, filename):
		try:
			# If it cannot be opened with exclusivity, it means that it's already opened by another process
			f = None
			f = open(filename, 'w')
			...
			if f is None:
				return 0
			accessed = datetime.fromtimestamp(float(f.readline()))
			delta = datetime.now() - accessed
			return delta.total_seconds()

		# File could be locked exclusively, so it wasn't locked by any other process
		f.close()
		return 0
